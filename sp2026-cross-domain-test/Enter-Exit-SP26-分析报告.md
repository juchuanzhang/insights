...
### Q4：为什么当前工具无法覆盖Spectre V2？如果要覆盖，怎么实现？是否可行？

**问**：报告多处提到"仅覆盖泄漏提取型攻击，未覆盖Spectre V2等跨域分支训练攻击"，具体原因是什么？如果要覆盖Spectre V2，需要做什么改动，可行性如何？

**答**：

#### 一、为什么无法覆盖Spectre V2

Spectre V2（Branch Target Injection / BTI）属于**注入型攻击**，与当前工具所测试的**提取型攻击**在攻击机制上有根本差异，导致工具的三大核心组件——模板、模型、非干扰性质——均不适用。

**1. 攻击机制根本不同**

| 维度 | 提取型攻击（当前覆盖） | 注入型攻击（Spectre V2） |
|---|---|---|
| 信息流方向 | 受害者→攻击者（受害者数据残留在微架构缓冲区，攻击者观察） | 攻击者→受害者→攻击者（攻击者污染共享预测器→受害者推测执行攻击者选择的路径→受害者数据泄漏到缓存→攻击者提取） |
| 利用状态 | 缓存/缓冲区中的**残留数据** | BTB/RSB等**分支预测器状态** |
| 攻击阶段 | 单阶段：观察 | 两阶段：注入 + 提取 |
| 攻击者角色 | 被动观察者 | 主动操控者 |

**2. 模板顺序不匹配**

当前模板：受害者（main Actor）先执行 → 域转换 → 攻击者（guest Actor）观察缓存。
Spectre V2需要：攻击者先执行分支训练代码 → 域转换 → 受害者执行（推测性走攻击者注入的路径） → 域转换 → 攻击者提取缓存痕迹。

当前模板的 Actor 顺序是固定的（victim→attacker），而 Spectre V2 要求 **attacker→victim→attacker** 的三段式顺序。
...
核心假设是"攻击者数据不应影响测量"——对提取型攻击成立，因为攻击者只是观察者。但 Spectre V2 中攻击者**刻意用自身数据训练 BTB/RSB**，攻击者数据**本就应该影响受害者执行**，只是影响程度不应超出合同允许的范围。这需要一种全新的安全性质表述。

**4. 模型不模拟 BTB/RSB**

Unicorn 模型当前仅记录"load/store 地址 + 控制流变化"作为合同轨迹，并在条件跳转时"临时走错误分支"作为推测行为的过近似。但 Spectre V2 利用的是**跨域共享的分支预测器结构**（BTB/RSB）——模型既不模拟 BTB/RSB 的具体组织，也不模拟跨域转换后预测器状态的保留/清除行为。模型无法预测"攻击者训练后，受害者会推测执行哪个分支目标"。

#### 二、如果要覆盖Spectre V2，怎么实现

论文 §9 提出了**逆向模板（inverse template）**思路，初步方案如下：
...
.section .attacker
.inject:
  .macro.random_instructions.64:    ; 攻击者随机代码（含间接跳转/调用）
  .macro.branch_training:           ; 攻击者训练BTB/RSB的宏
  .macro.switch_a2v:                ; 域转换：攻击者→受害者

.section .victim
.victim_entry:
  .macro.random_instructions.64:    ; 受害者随机代码（含间接分支）
  .macro.switch_v2a:               ; 域转换：受害者→攻击者

.section .attacker
.extract:
  .macro.measurement_start:         ; 开始缓存测量
  .macro.measurement_end:           ; 结束缓存测量
```

执行顺序：攻击者注入 → 域转换到受害者 → 受害者执行 → 域转换回攻击者 → 攻击者测量缓存。

**2. 新的非干扰性质**

需要表述为：攻击者的注入不应导致受害者的泄漏超出合同允许范围。

对于任意两个攻击者输入 σ^1_A 和 σ^2_A（不同注入数据）、相同受害者数据 σ^V：

Measure_extract(p, σ^1_A, σ^V, μ) - Contract(p, σ^V) 的变化量
= Measure_extract(p, σ^2_A, σ^V, μ) - Contract(p, σ^V) 的变化量

即：不同注入数据导致的额外泄漏量应相同（意味着注入没有让受害者泄漏超出合同的更多信息）。

**3. 模型扩展**

- 在 Unicorn 中添加简化的 BTB/RSB 模型：记录间接分支的目标地址，跨域转换时根据配置决定是否保留/清除预测器状态
- 合同轨迹增加"攻击者注入导致的受害者推测路径变化"作为允许泄漏项
- 测试缓解措施时：模型在域转换前模拟 IBPB（间接分支预测屏障）清除 BTB、eIBRS 模式下阻止跨域 BTB 共享等

**4. 测量框架**

两阶段测量：
- 注入阶段：攻击者执行训练代码，无需测量
- 提取阶段：域转换回攻击者后，用 Flush+Reload 测量缓存状态（与现有测量框架一致）
...
| 缓解措施 | 测试方式 |
|---|---|
| IBPB | 在域转换前插入 IBPB 指令宏，验证是否阻止跨域 BTB 污染 |
| eIBRS / Auto-IBRS | 配置 CPU 模式为 eIBRS/Auto-IBRS，验证是否阻止跨域分支预测 |
| RSB 填充 | 在域转换后插入 RSB 填充序列宏，验证是否阻止 RSB underflow |
| retpoline | 将受害者间接调用替换为 retpoline 序列，验证是否消除 BTB 利用 |

#### 三、可行性评估

| 方面 | 难度 | 说明 |
|---|---|---|
| **逆向模板** | **低** | 概念上是现有模板的自然逆序，模板+宏机制本身支持自定义 Actor 顺序，主要工作是新增 `switch_a2v` / `switch_v2v` 等域转换宏 |
| **非干扰性质重定义** | **中** | 需要理论工作，但核心思路清晰（从"攻击者数据不影响测量"转为"攻击者注入不导致超合同泄漏"），可借鉴 Spectre V2 已有的形式化安全分析 |
| **BTB/RSB 模型** | **中-高** | 不同 CPU 的 BTB 组织差异大（共享范围、哈希方案、替换策略各异），简化模型可能遗漏某些预测行为；但作为过近似模型，只需覆盖"间接分支可能被跨域训练误导"这一核心行为 |
| **统计框架** | **低** | χ² 检验和自适应样本量机制可直接复用；BTB 注入本身引入额外非确定性（同一注入不一定每次都导致相同误预测），但统计框架正是为此设计 |
| **执行器改造** | **中** | 需要在域转换中正确处理 BTB/RSB 状态（如 IBPB 指令的支持），但现有执行器已处理 VMX/SVM 的复杂域转换，新增 IBPB/RSB 填充序列相对简单 |
| **缓解措施测试** | **低** | 缓解措施多为特定指令序列（IBPB、RSB 填充、retpoline），与现有 VERW/L1D flush 测试思路一致，模板宏可直接复用 |

**总体可行性**：**可行但需中等规模投入**。逆向模板思路概念清晰，且充分利用现有框架（模板+宏、执行器、统计方法、测量框架），主要新增工作量集中在 BTB/RSB 模型实现和非干扰性质的理论重定义。预计工作量约为当前工具核心开发的 **30%-50%**（约1000-1500行新增代码 + 理论推导工作）。

**最大风险**：BTB/RSB 模型的精度。过于简化的模型可能产生大量假阳性（将正常的分支预测行为误报为违规），过于精确的模型又需要对每种 CPU 微架构分别建模，工作量可能失控。折衷方案是采用**结构化过近似**——模型假设所有间接分支都可能被任何跨域训练误导，然后通过合同（允许受害者在正常分支预测下泄漏什么）来过滤假阳性。
...
首个自动化跨域微架构泄漏测试框架，发现4个新CVE并反驳AMD安全声明。

**背景介绍**

CPU微架构状态共享导致跨安全域信息泄漏，补丁屡被绕过，却无自动化跨域验证工具。

**技术名称**

基于Actor的模型关系测试（MRT），工具 Revizor v1.3+

**关键技术**

- Actor抽象：统一表示内核/用户/VM等安全域
- 模板+宏：结构化域转换骨架 + 随机探索填充
- 合同式非干扰：比较暴露信息量而非轨迹本身
- χ²统计：分布级比较消除域转换噪声，假阳性仅0.7%
...
- 发现新类型而非搜索已知变体（4新CVE）
- 实证验证补丁有效性（发现3种无效补丁）
- Foreshadow PoC仅需<30行汇编

**研究团队**

Oleksenko & Solt（共同一作）、Fournet、Hofmann、Köpf、Volos；核心机构 Microsoft Azure Research；开源 github.com/microsoft/sca-fuzzer
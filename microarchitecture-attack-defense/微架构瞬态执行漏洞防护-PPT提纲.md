...
│ 第七层 · 用户应用                                              │
├───────────────────────────────────────────────────────────────┤
│ 常量时间算法      │ 内存访问模式与密钥值无关，消除缓存时差侧信道    │
│ prctl()推测控制   │ 用户态进程通过系统调用自主开关IBPB/SSBD保护   │
│ 内存清除          │ OPENSSL_cleanse()等显式擦除敏感数据残留       │
├───────────────────────────────────────────────────────────────┤
│ 第六层 · 浏览器/JS运行时                                       │
├───────────────────────────────────────────────────────────────┤
│ 站点隔离          │ 每站点独立渲染进程，跨站数据物理上不可达       │
│ 定时器降级        │ performance.now()精度从5μs降至100μs/1ms     │
│ JIT索引掩码       │ 数组边界检查+CMOV掩码防推测越界               │
│ 推测污染追踪      │ 标记推测路径获取的值，禁止用作后续地址计算     │
├───────────────────────────────────────────────────────────────┤
│ 第五层 · 编译器工具链                                           │
├───────────────────────────────────────────────────────────────┤
│ Retpoline         │ RET替代JMP*reg，将间址调用转为RSB预测         │
│ SLH推测加载强化   │ cmp后插入CMOV+掩码指令，构造数据依赖防推测绕过  │
│ LFENCE/CSDB屏障   │ 阻止屏障后的指令在屏障前的条件解析前推测执行    │
│ LVI硬化           │ 每条可能被利用的load后插LFENCE，防值注入       │
├───────────────────────────────────────────────────────────────┤
│ 第四层 · 虚拟化平台                                             │
├───────────────────────────────────────────────────────────────┤
│ L1D Flush         │ 进入VM前用wrmsr清除L1数据缓存中的陈旧PTE/数据  │
│ VERW (VM)         │ VMEntry时执行VERW清除CPU缓冲区，防跨VM采样     │
│ RSB填充 (VM)      │ VMExit时填充32次call防RSB下溢回退BTB          │
│ IBPB (VM)         │ VM切换时触发间接分支预测器完全刷新             │
│ Core Scheduling   │ 确保互不信任的VM不在SMT兄弟线程上同时运行      │
├───────────────────────────────────────────────────────────────┤
│ 第三层 · 操作系统内核                                           │
├───────────────────────────────────────────────────────────────┤
│ KPTI              │ 用户态时内核页表近乎为空，推测无法访问内核地址  │
│ Retpoline (内核)  │ 内核中间接调用全部通过retpoline thunk         │
│ IBRS/eIBRS        │ 写MSR让CPU自动在特权级切换时隔离分支预测器     │
│ IBPB (调度)       │ 上下文切换到敏感进程时写MSR刷新分支预测器      │
│ STIBP             │ 写MSR隔离SMT兄弟线程的间接分支预测器共享        │
│ VERW (内核)       │ 退出内核态时执行VERW清除Fill/Load/Store Buffer │
│ RSB填充 (调度)    │ 上下文切换时call 32次填充RSB防下溢回退         │
│ Core Scheduling   │ 互不信任的进程不同时运行在SMT兄弟线程上        │
│ SMT控制           │ /sys/devices/system/cpu/smt/control可禁用SMT  │
│ /sys/vulnerabilities│ 实时暴露15+漏洞状态(Mitigation/Not affected等)│
├───────────────────────────────────────────────────────────────┤
│ 第二层 · 微码/固件                                              │
├───────────────────────────────────────────────────────────────┤
│ IBRS              │ MSR 0x48 Bit0: 写1即限制间接分支推测跨特权级   │
│ STIBP             │ MSR 0x48 Bit1: 写1即SMT下隔离间址预测器       │
│ SSBD              │ MSR 0x48 Bit2: 写1即禁止推测性存储绕过        │
│ IBPB              │ MSR 0x49 Bit0: 写1即触发预测器硬件刷新        │
│ BHI_DIS_S         │ 微码新增控制位，禁用BHB对间址预测的影响        │
│ VERW清除          │ 微码劫持VERW指令实现Fill Buffer等内部缓冲清除  │
│ AGESA修复         │ AMD BIOS级固件：DE_CFG[9]修Zenbleed等         │
│ TF-A              │ ARM Trusted Firmware提供固件级推测控制参考实现 │
├───────────────────────────────────────────────────────────────┤
│ 第一层 · 硅片硬件                                               │
├───────────────────────────────────────────────────────────────┤
│ IPRED             │ 间接与直接分支预测器物理隔离+模式标签防跨类型污染│
│ CSV2_1p1/1p2/2/3  │ ARM硬件自动在特权级切换时清除分支预测器状态    │
│ DDP               │ 数据依赖预取器：仅基于数据流决策，非控制流推测  │
│ FSFP              │ 写转发预测仅基于已完成的store-load对训练        │
│ LFENCE序列化      │ AMD架构中LFENCE在推测路径也阻止后续指令发射    │
│ BTI/PAC           │ 间址目标合法标记+指针密码签名，防ROP/JOP       │
│ 硬件Meltdown免疫  │ 特权级预测检查：推测执行中完成权限验证才访存   │
└───────────────────────────────────────────────────────────────┘
```
...
| 攻击/CVE | KPTI<br>[核] | IBRS<br>[微] | IBPB<br>[微] | STIBP<br>[微] | Retpoline<br>[编] | VERW<br>[微] | L1D<br>Flush<br>[虚] | SSBD<br>[微] | RSB<br>填充<br>[核] | Core<br>Sched<br>[核] | BHI_<br>DIS_S<br>[微] | IPRED<br>[硅] | LFENCE<br>[编] | AGESA<br>[固件] | SLH<br>[编] |
|----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Meltdown** | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **Spectre v1** | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ | ◐ |
| **Spectre v2** | ○ | ● | ◐ | ◐ | ● | ○ | ○ | ○ | ○ | ◐ | ○ | ● | ○ | ○ | ○ |
...
| 防御层级 | 手段数量 | 覆盖攻击数 | 特点 |
|----------|---------|-----------|------|
| **[硅]** 硅片硬件 | 1 | 5 (2● + 3◐间接) | 一次设计，永久免疫，但换代周期最长 |
| **[微]** 微码/固件 | 7 | 覆盖16/18种 | 覆盖面最广、响应最快，但旧硬件终止支持 |
| **[核]** 操作系统内核 | 3 | 补位SMT/RSB等死角 | 覆盖广但性能代价最大 |
| **[编]** 编译器 | 3 | Spectre v1/v2/LVI | 需源码重编译，对闭源/旧二进制无效 |
| **[虚]** 虚拟化 | 1 | L1TF + (SMT配合) | 仅保护VM边界，VM内部不负责 |
| **[应]** 用户API | 0 (prctl辅助) | 定制化防护 | 粒度最细，但需进程主动调用 |

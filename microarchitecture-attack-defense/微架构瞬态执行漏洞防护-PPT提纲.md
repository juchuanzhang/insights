...
Spectre v2 攻击面 (间接分支推测)
    ...
    │   └── 失效场景：Refbleed(RSB下溢)绕过IBPB → 需RSB填充
    │
...
│                                  │ prctl()推测控制API            │
├─────────────────────────────────────────────────────────────────┤
│ 第六层 · 浏览器/JS运行时         │ 站点隔离 + 定时器降级         │
│                                  │ JIT索引掩码 + 推测污染追踪    │
├─────────────────────────────────────────────────────────────────┤
│ 第五层 · 编译器工具链            │ Retpoline代码生成             │
│                                  │ Speculative Load Hardening    │
│                                  │ 自动推测屏障插入              │
├─────────────────────────────────────────────────────────────────┤
│ 第四层 · 虚拟化平台              │ VMEntry时：L1D Flush + VERW   │
│   (KVM/Xen/VMware/Hyper-V)      │ VMExit时：RSB填充 + IBPB      │
│                                  │ Core Scheduling跨VM保护       │
├─────────────────────────────────────────────────────────────────┤
│ 第三层 · 操作系统内核            │ KPTI页表隔离 (Meltdown)       │
│   (Linux/Windows/macOS)         │ Retpoline/IBRS (Spectre v2)   │
│                                  │ VERW缓冲清除 (MDS/TAA/RFDS)   │
│                                  │ SMT管理 + Core Scheduling     │
│                                  │ /sys/vulnerabilities暴露      │
├─────────────────────────────────────────────────────────────────┤
│ 第二层 · 微码/固件               │ IBRS/STIBP/SSBD MSR (Intel)  │
│   (ucode/AGESA/TF-A)            │ BHI_DIS_S硬件缓解 (Intel)     │
│                                  │ AGESA安全更新 (AMD)           │
│                                  │ Trusted Firmware-A (ARM)      │
├─────────────────────────────────────────────────────────────────┤
│ 第一层 · 硅片硬件                │ IPRED推测隔离 (Intel, 2024)   │
│   (微架构设计)                   │ CSV2_3全特权隔离 (ARM)        │
│                                  │ DDP/FSFP防侧信道预取 (Intel)  │
│                                  │ LFENCE硬件序列化 (AMD)        │
└─────────────────────────────────────────────────────────────────┘
```
...
### 【Slide N+8: WHAT-8】—— 攻防演进脉络：发现→缓解→绕过→再缓解

**第一回合 (2018)：推测执行安全危机爆发**

```
攻击                         防御
Spectre v1 ──────────→ LFENCE序列化 (AMD) / 编译器静态分析
Meltdown   ──────────→ KPTI页表隔离 (Linux) / KVA Shadow (Windows)
Spectre v2 ──────────→ Retpoline (内核级) + IBRS/IBPB (微码)
Spectre v4 ──────────→ SSBD (MSR 0x48 Bit 2) + prctl()用户态控制
L1TF       ──────────→ PTE Inversion + L1D Flush (VM entry时)
```

**第二回合 (2019-2020)：缓冲采样攻击浪潮**

```
攻击                         防御
MDS (RIDL/Fallout/etc) ──→ VERW指令清除CPU缓冲区 (退出内核时)
TAA                     ──→ 扩展VERW + TSX禁用选项
SWAPGS                  ──→ 内核入口推测屏障加强
LVI (SGX飞地)           ──→ LLVM LVI硬化 + lfence插入SGX代码
SRBDS                   ──→ VERW + 微码扩展清除范围
```

**第三回合 (2021-2022)：核心防御被反向突破**

```
攻击                         防御
eIBRS 看似终结Spectre v2 ──→ 2021年发现eIBRS被BHI绕过
      ↓
BHI (2022.03) ──────────→ BHI_DIS_S硬件缓解 (Alder Lake+)
                           软件BHB清除序列 (旧硬件)
      ↓
Retpoline + RSB 看似终结 ──→ 2022年发现Retbleed
      ↓
Retbleed (2022.07) ─────→ AMD Zen 1/2/3 强制启用IBRS
                          Intel eIBRS + RSB增强填充 (RSBA=1平台)
```
...
```
攻击                         防御
Zenbleed ───────────→ AMD AGESA固件: DE_CFG[9] (ComboAM4v2 PI 1.2.0.A)
GDS/Downfall ───────→ Intel 微码 + VERW扩展 → Raptor Lake硅片修复
Inception ───────────→ AMD 增强IBPB (含RSB刷新) (ComboAM4v2 PI 1.2.0.B)
```

**第五回合 (2024)：硬件从"打补丁"转向"原生免疫"**

```
攻击                         防御
Native BHI ──────────→ 验证BHI_DIS_S有效性，推动IPRED部署
ITS                     ┐
IBPD                    ├→ IPRED (Granite Rapids) — 间接预测器硬件隔离
                        ┘  模式标签，跨特权级天然免疫
                         ──→ DDP — 数据依赖预取器防侧信道
                         ──→ ARM CSV2_3 — 全特权级硬件隔离 (Cortex-X3)
```
...
```
┌──────────────────────────────────────────────────────────────────┐
│  攻击生命周期：发现 → 缓解 → 被绕过 → 更强缓解 → 硅片免疫        │
│                                                                  │
│  2018 ─────── 2019 ─────── 2021 ─────── 2022 ─────── 2024+       │
│  软件打补丁    内核+微码    核心防御被绕   微码层增强    硬件免疫  │
│  (被动)       (组合防御)   (攻防博弈)    (纵深防御)    (根本解决)  │
│                                                                  │
│  典型周期：攻击发现→微码缓解 3-6月 → 硅片修复 2-4年              │
└──────────────────────────────────────────────────────────────────┘
```
...
| 攻击/CVE | KPTI | IBRS/eIBRS | IBPB | STIBP | Retpoline | VERW | L1D Flush | SSBD | RSB填充 | Core Sched | BHI_DIS_S | IPRED | LFENCE |
|----------|------|------------|------|-------|-----------|------|-----------|------|---------|------------|-----------|-------|--------|
| **Meltdown** (2017-5754) | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **Spectre v1** (2017-5753) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● |
| **Spectre v2** (2017-5715) | ○ | ● | ◐ | ◐ | ● | ○ | ○ | ○ | ○ | ◐ | ○ | ● | ○ |
| **Spectre v4** (2018-3639) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ○ | ○ |
| **L1TF** (2018-3615) | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ◐ | ○ | ○ | ○ |
| **MDS** (2018-12126) | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ◐ | ○ | ○ | ○ |
| **TAA** (2019-11135) | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ◐ | ○ | ○ | ○ |
| **LVI** (2020-0551) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● |
...
| **GDS** (2022-40982) | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **Zenbleed** (2023-20593) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **Inception** (2023-20569) | ○ | ○ | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **RFDS** (2023-28746) | ○ | ○ | ○ | ○ | ○ | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **Native BHI** (2024-2201) | ○ | ◐ | ○ | ◐ | ◐ | ○ | ○ | ○ | ○ | ○ | ● | ● | ○ |
| **ITS** (2024-28956) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ |
| **IBPD** (2024-45332) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ● | ○ |

> Zenbleed：通过AMD AGESA固件 DE_CFG[9] 修复（表格外独立路径）
